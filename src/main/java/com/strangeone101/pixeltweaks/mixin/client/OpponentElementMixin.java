...
public abstract class OpponentElementMixin extends PixelmonWidget {
    ...
    @Shadow(remap = false) private static ResourceLocation OPPONENT;
    @Shadow(remap = false) private static ResourceLocation HEALTHY;
    @Shadow(remap = false) private static ResourceLocation CAUTION;
    @Shadow(remap = false) private static ResourceLocation WARNING;
    @Shadow(remap = false) private static ResourceLocation SHINY;
    @Shadow(remap = false) private static ResourceLocation CAUGHT;
    @Shadow(remap = false) private static ResourceLocation TARGET;

    @Shadow(remap = false) private Screen parent;
    @Shadow(remap = false) private PixelmonClientData enemy;
    @Shadow(remap = false) private ScreenParticleEngine particleEngine;
    ...
    public void drawElement(GuiGraphics graphics, float scale) {
        ...
        ScreenHelper.drawBar(
                ...
                10.0D,
                healthPercent,
                this.enemy.getHealthColor(this.enemy.health.intValue(), this.enemy.maxHealth)
        );

        ScreenHelper.drawImage(
                healthPercent <= 0.5F ? (healthPercent <= 0.25F ? WARNING : CAUTION) : HEALTHY,
                graphics,
                (float) (this.x - 10),
                (float) (this.y - 18),
                60.0F,
        ...
        pose.translate((double) (x - ix), (double) (y - iy), (double) zLevel);
        graphics.blit(texture, ix, iy, 0F, 0F, 40, 40, 40, 40);
        ...
        this.particleEngine.drawAtOffset(
                graphics,
                this.enemy.pokemonUUID.toString(),
                (double) (this.x + 5),
                (double) (this.y + 1),
                (double) RandomHelper.getRandom().nextInt(26),
                (double) RandomHelper.getRandom().nextInt(26)
        );
        ...
        if (this.enemy.getGender() != Gender.NONE) {
            ScreenHelper.drawImage(
                    this.enemy.getGender() == Gender.MALE ? Resources.male : Resources.female,
                    graphics,
                    (float) (this.x + 52) + offset,
                    (float) (this.y + 5),
                    5.0F,
                    8.0F,
                    this.zLevel
        ...
        if (PixelmonConfigProxy.getGraphics().isAdvancedBattleInformation()) {
            ...
            String percentage = df.format(healthPercent * 100.0D).replace(".0", "");
        ...
        if (this.enemy.status != -1 && StatusType.getEffect(this.enemy.status) != null) {
            float[] texturePair = StatusType.getTexturePos(StatusType.getEffect(this.enemy.status));
            ScreenHelper.simpleDrawImageQuad(
                    Resources.status,
                    graphics,
                    (float) (this.x + 54 + ScreenHelper.getStringWidth(this.enemy.getDisplayName())) + offset,
                    (float) (this.y + 4),
                    10.5F,
                    10.5F,
                    texturePair[0] / 768.0F,
                    texturePair[1] / 768.0F,
                    (texturePair[0] + 240.0F) / 768.0F,
                    (texturePair[1] + 240.0F) / 768.0F,
                    this.zLevel
            );
        }
        ...
        if (ClientProxy.battleManager.catchCombo != 0) {
            ScreenHelper.drawScaledString(
                    graphics,
                    I18n.get("gui.battle.catch_combo", ClientProxy.battleManager.catchCombo),
                    (float) (this.x + 22) + offset,
                    (float) this.y + 38.75F,
                    -986896,
                    12.0F
            );
        ...
        if (shiny && RandomHelper.getRandom().nextInt(80) == 0) {
            ...
            this.particleEngine.addParticle(
                    new ScreenParticleEngine.GuiParticle(
                            this.enemy.pokemonUUID.toString(),
                            SHINY,
                            0.0D, 0.0D, 1.0D,
                            0.0D, 0.0D, 0.0D,
                            ...
                            120,
                            (particle, guiGraphics) -> {
                                int age = particle.age;
                                int max = particle.maxAge;
                                int half = max / 2;
                                particle.a = (float) (age <= half ? age : half - (age - half)) / (float) half;
                            }
                    )
            );
        }
